...
int gm10_response_reader( struct gm10_comm *comm)
{
  ...
  if( comm->inbuffer[0] != 'E')
    {
      ...
      // THIS MIGHT TIMEOUT!!!
      read( comm->sockfd, comm->inbuffer, GM10_INBUFLEN);
  ...
  switch( comm->inbuffer[1])
    ...
    case '2': // possibly handle correctly in future, only GM10 uses
      if( gm10_ok_error_reader( comm) )
        return RESPONSE_INVALID;
